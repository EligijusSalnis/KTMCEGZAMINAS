...
app.post("/api/register", async (req, res) => {
  ...
  if (existingUser) {
    return res.status(400).send({
      message: "Vartotojas su tokiu el. pašto adresu jau egzistuoja.",
    });
  }
  ...
  try {
    ...
    const user = await Vartotojai.create({
      vardas,
      elPastas,
...
app.post("/api/login", async (req, res) => {
  ...
  const token = jwt.sign(
    { id: vartotojas.id, isAdmin: vartotojas.isAdmin },
    "secretkey",
    { expiresIn: "1h" }
  );
  res.json({ token });
...
app.put("/api/renginiai/:id", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!req.user.isAdmin) {
      return res
        .status(403)
        .json({ message: "Jūs neturite teisių redaguoti šio renginio" });
    }
...
app.delete("/api/renginiai/:id", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!req.user.isAdmin) {
      return res
        .status(403)
        .json({ message: "Jūs neturite teisių ištrinti šio renginio" });
    }
...
// Pridėti įvertinimą
app.post("/api/ivertinimai", authenticateJWT, async (req, res, next) => {
  try {
    const { ivertinimas, RenginiaiId } = req.body;
    const rating = await Ivertinimai.create({
      ivertinimas,
...
app.get("/api/renginiai/:id/ivertinimai", authenticateJWT, async (req, res) => {
  const { id } = req.params;

  try {
    const ivertinimai = await Ivertinimai.findAll({
      where: { RenginiaiId: id },
      attributes: [
        [sequelize.fn("AVG", sequelize.col("ivertinimas")), "averageRating"],
      ],
    });
    if (
      ivertinimai.length > 0 &&
      ivertinimai[0].dataValues.averageRating !== null
    ) {
      res.json(ivertinimai[0].dataValues.averageRating);
    } else {
      res.json({ message: "Vidutinis įvertinimas nerastas" });
    }
  } catch (error) {
    res.status(500).json({ message: "Klaida gaunant įvertinimus", error });
  }
});
...
app.post("/api/renginiai", authenticateJWT, async (req, res, next) => {
  try {
    const { pavadinimas, kategorija, laikas, vieta, nuotrauka } = req.body;
...
app.get("/protected", authenticateJWT, (req, res) => {
  res.send({
    message: "Ši informacija prieinama tik prisijungusiems vartotojams.",
  });
});
